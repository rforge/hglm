`GLM.MME` <-
function(Augy, AugXZ, starting.delta, tau, phi, n.fixed, n.random,
  ...
  while(maxmuit<=maxit){
    ...
    if (!is.null(z)) { 
      eta.i<-as.numeric(x%*%est[1:(p-k)]+z%*%est[(p-k+1):p])
      mu.i<-family$linkinv(eta.i)
      dmu_deta<-family$mu.eta(eta.i)
      zi<-eta.i+(y-mu.i)/dmu_deta
      v.i<-est[(p-k+1):p]
